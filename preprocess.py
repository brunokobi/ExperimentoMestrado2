...
def load_and_save2(category, filename, dataset, dataset_folder, shape):
    ...
    for line in ls:
        ...
        start, end, indx = int(
            pos.split('-')[0]), int(pos.split('-')[1]), [int(i)-1 for i in values]
...
def load_data(dataset):
    ...
    if dataset == 'synthetic':
        ...
        dataset_folder = 'data/BRUNO'
        file_train = os.path.join(dataset_folder, 'dadosbrutos6.json')
        file_test = os.path.join(dataset_folder, 'dadosbrutos48.json')

        df_train = pd.read_json(file_train, lines=True)[
            ['P-JUS-CKGL']][4000:6000]
        print('train', df_train)

        df_test = pd.read_json(file_test, lines=True)[['P-JUS-CKGL']][1:]
        print('test', df_test)

        train, min_a, max_a = normalize2(df_train.values)
        test, _, _ = normalize2(df_test.values, min_a, max_a)

        labels = pd.read_json(file_test, lines=True)[
            ['class']][1:] + 0
        print('labels', labels)
        ...
        for filename in file_list:
            if filename.endswith('.txt'):
                load_and_save('train', filename, filename.strip(
                    '.txt'), dataset_folder)
                s = load_and_save('test', filename,
                                  filename.strip('.txt'), dataset_folder)
                load_and_save2('labels', filename, filename.strip(
                    '.txt'), dataset_folder, s)
    elif dataset == 'UCR':
        ...
        for filename in file_list:
            ...
            train, test, labels = train.reshape(
                -1, 1), test.reshape(-1, 1), labels.reshape(-1, 1)
            for file in ['train', 'test', 'labels']:
        ...
        for filename in file_list:
            ...
            for timestamp in labeldict['realKnownCause/'+filename]:
                ...
                index = np.where(
                    ((df['timestamp'] == tstamp).values + 0) == 1)[0][0]
        ...
        _, min_a, max_a = normalize3(
            np.concatenate((df_train, df_test), axis=0))
        ...
        for file in ['train', 'test', 'labels']:
            np.save(os.path.join(folder, f'{file}.npy'), eval(
                file).astype('float64'))
    elif dataset == 'SWaT':
        ...
        for fn in filenames:
            ...
            labels = np.zeros(test.shape)
            indices = values[values['chan_id'] ==
                             fn]['anomaly_sequences'].values[0]
        ...
        train = pd.read_csv(os.path.join(
            dataset_folder, 'WADI_14days.csv'), skiprows=1000, nrows=2e5)
        ...
        for index, row in ls.iterrows():
            ...
            labels.loc[(labels['Time'] >= st) & (
                labels['Time'] <= et), matched] = 1
        train, test, labels = convertNumpy(
            train), convertNumpy(test), convertNumpy(labels)
        ...
        train, test = train.values[1:, 1:].astype(
            float), test.values[1:, 1:].astype(float)
        train, min_a, max_a = normalize3(train)
        test, _, _ = normalize3(test, min_a, max_a)
